...
import { MoreVertical, RefreshCw, CheckCircle2 } from 'lucide-react';
...
interface HeaderProps {
  ...
  onFetchInbox: () => void;
  sensorActiveCount: number;
  sensorAnomalyScore: number;
  sensorPhiSynchronicity: boolean;
      ...
      <div className="flex gap-4 items-center">
        ...
        {sensorActiveCount > 0 && (
          ...
          >
            ...
            />
            <span
              className={`text-[10px] font-mono font-bold uppercase ${sensorPhiSynchronicity ? 'text-yellow-400 animate-pulse' : sensorAnomalyScore > 0.4 ? 'text-red-400' : 'text-emerald-400'}`}
            >
              {sensorPhiSynchronicity
                ? 'Φ SYNC'
