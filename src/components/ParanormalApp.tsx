...
const Starfield = React.forwardRef<HTMLDivElement, Record<string, never>>((props, ref) => {
...
function ChatTab({ settings }: { settings: Settings }) {
  ...
  const [chatLog, setChatLog] = useState<{role: string, text: string, attachments?: LocalAttachment[]}[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setChatLog(prev => [...prev, { role: 'sys', text: `ERROR: ${msg}` }]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {['google', 'ollama', 'openRouter', 'grok'].map(p => (
          <button 
            key={p}
            onClick={() => setProvider(p as 'ollama' | 'google' | 'grok' | 'openRouter')}
          ...
          onChange={(e) => {
             if (e.target.files && e.target.files.length > 0) {
               const newAttachments = Array.from(e.target.files).map(f => {
                 ...
                 return {
                   type: type as 'document' | 'image' | 'video' | 'audio',
...
// ---- Data / Settings Tab ----
function DataTab({ settings, setSettings }: { settings: Settings, setSettings: (val: Settings) => void }) {
  const fields = [
...
function SensorsTab({ settings }: { settings: Settings }) {
  ...
  useEffect(() => {
    ...
    const fetchWeather = async () => {
      try {
        ...
        await swRes.json(); // Usually a JSON object or array, fallback mock if failing
  ...
  const handleOpenFile = async (file: { type: string, url: string, path: string }) => {
  ...
  const handleFetchLocal = async () => {
    ...
    try {
      ...
      alert("Local scan failed. Ensure VisionLLM server is running on port 8000. " + (err instanceof Error ? err.message : String(err)));
    } finally {
      setLoading(false);
    }
  };
...
function ASCIIFeed({ isCamActive, filterMode }: { isCamActive: boolean, filterMode: string }) {
  ...
  useEffect(() => {
    const draw = () => {
      ...
      if (filterMode === 'ascii') {
        ...
        // Draw lower res for performance
        const w = 400;
        const h = 300;
        const tempCanvas = document.createElement('canvas');
        tempCanvas.width = w;
        ...
        if (tCtx && videoRef.current) {
          ...
          if (filterMode === 'monochrome') {
            ...
            for (let i = 0; i < data.length; i += 4) {
              const b = (data[i] + data[i+1] + data[i+2]) / 3;
              if (b < 85) { 
                data[i] = 0; data[i+1] = 0; data[i+2] = b * 3;
              } else if (b < 170) {
                data[i] = (b - 85) * 3; data[i+1] = 0; data[i+2] = 255 - (b - 85) * 3;
              } else {
                data[i] = 255; data[i+1] = (b - 170) * 3; data[i+2] = 0;
              }
            }
            ...
            for (let y = 1; y < h - 1; y++) {
               for (let x = 1; x < w - 1; x++) {
                  const idx = y * w4 + x * 4;
                  const b0 = (copy[idx] + copy[idx+1] + copy[idx+2])/3;
                  const bx = (copy[idx + 4] + copy[idx+5] + copy[idx+6])/3;
                  const by = (copy[idx + w4] + copy[idx+w4+1] + copy[idx+w4+2])/3;
                  const diff_x = b0 - bx;
                  const diff_y = b0 - by;
                  const edge = Math.sqrt(diff_x*diff_x + diff_y*diff_y) * 2;
                  
                  const c = edge > 30 ? 255 : 0;
                  data[idx] = 0;
                  data[idx+1] = c;
                  data[idx+2] = c; 
               }
          ...
          ctx.globalAlpha = 1.0;
          ctx.filter = filterMode === 'monochrome' ? 'contrast(1.5)' : 'none';
...
function OpticsTab() {
  ...
  const [filterMode, setFilterMode] = useState('ascii');

  useEffect(() => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const tel = (window as any).paranormalTelemetry || {};
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).paranormalTelemetry = { ...tel, isCamActive };
  }, [isCamActive]);

  return (
    <div className="flex flex-col h-full">
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-4 border-b border-[#22d3ee]/20 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] text-[#22d3ee]/50 mt-1 uppercase tracking-widest">Motorola G5 13MP / OpenCV Monocular Estimation</span>
          </div>
        </div>
        <div className="ml-auto flex items-center gap-3 w-full md:w-auto">
          <select 
            ...
            className="bg-black/40 border border-[#22d3ee]/30 text-[#22d3ee] text-[10px] uppercase font-bold tracking-widest px-2 py-1.5 rounded focus:outline-none"
          >
          ...
          <button 
            ...
            className={`text-[9px] font-bold tracking-widest px-3 py-1.5 border rounded transition-all shrink-0 ${
              isCamActive 
      ...
      <div className="flex-1 relative rounded-[24px] p-2 border border-[#22d3ee]/20 bg-[#050505]/40 backdrop-blur-xl overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.1),0_0_30px_rgba(34,211,238,0.05)] text-white flex items-center justify-center">
        ...
        <ASCIIFeed isCamActive={isCamActive} filterMode={filterMode} />
...
function DeviceSensorsTab() {
  ...
  useEffect(() => {
    let magSensor: ReturnType<typeof setInterval> | { start: () => void, stop: () => void, addEventListener: (e:string, cb:()=>void) => void, x: number, y: number, z: number } | null = null;
    ...
    const setupSensors = async () => {
      ...
      if ('Magnetometer' in window) {
        try {
          magSensor = new (window as unknown as { Magnetometer: new (opts: { frequency: number }) => { start: () => void, stop: () => void, addEventListener: (e:string, cb:()=>void) => void, x: number, y: number, z: number } }).Magnetometer({ frequency: 10 });
          magSensor.addEventListener('reading', () => {
            ...
            setEmfHistory(prev => [...prev.slice(-49), mag]);
          });
          magSensor.start();
        } catch {
  ...
  useEffect(() => {
    ...
    const tel = (window as any).paranormalTelemetry || {};
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).paranormalTelemetry = { ...tel, emfMagnitude: magnetometer ? emfMagnitude : undefined };
  }, [magnetometer, emfMagnitude]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex justify-between items-center mb-6 border border-[#f59e0b]/20 p-4 rounded-xl bg-black/40 shadow-[0_0_15px_rgba(245,158,11,0.05)]">
         <div className="flex flex-col">
           <span className="text-xs tracking-widest uppercase text-[#f59e0b]/70 font-bold">Moto G5 Sensorhub</span>
           <span className="text-[9px] text-[#f59e0b]/50 tracking-widest">STM32L051 Controller Linked</span>
         </div>
         <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#050505]/40 border border-[#f59e0b]/20 p-6 rounded-[24px]">
          <h3 className="text-[10px] text-[#f59e0b]/50 uppercase tracking-widest mb-1">BMI160 Accelerometer</h3>
          <p className="text-[8px] text-[#f59e0b]/40 mb-3 tracking-widest">(G-Force / Microvibrations)</p>
        ...
        {/* Gyroscope */}
        <div className="bg-[#050505]/40 border border-[#f59e0b]/20 p-6 rounded-[24px]">
          <h3 className="text-[10px] text-[#f59e0b]/50 uppercase tracking-widest mb-1">BMI160 Gyroscope</h3>
          <p className="text-[8px] text-[#f59e0b]/40 mb-3 tracking-widest">(Rotation / Game Vector)</p>
        ...
        <div className="md:col-span-2 bg-[#050505]/40 border border-red-500/20 p-6 rounded-[24px]">
          <div className="flex justify-between items-center mb-4">
             <div className="flex flex-col">
               <h3 className="text-[10px] text-red-500/50 uppercase tracking-widest">AKM09912 Magnetometer</h3>
               <p className="text-[8px] text-red-500/40 tracking-widest mt-1">(EMF Approximation - No native thermal)</p>
             </div>
          ...
          <div className="h-10 mt-4 border border-white/5 bg-black/40 rounded overflow-hidden relative">
             <svg width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 49 100">
               <polyline 
                 points={emfHistory.map((val, i) => `${i},${100 - Math.min(100, (val / 150) * 100)}`).join(' ')} 
                 fill="none" 
                 stroke="#ef4444" 
                 strokeWidth="1.5"
...
function AudioTab() {
  ...
  const [spiritWords, setSpiritWords] = useState<{word: string, timestamp: number, id: number}[]>([]);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const streamRef = useRef<MediaStream | null>(null);
  const audioCtxRef = useRef<AudioContext | null>(null);
  const analyserRef = useRef<AnalyserNode | null>(null);
  const recognitionRef = useRef<{ stop: () => void, start: () => void, onresult: unknown } | null>(null);

  useEffect(() => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const tel = (window as any).paranormalTelemetry || {};
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).paranormalTelemetry = { 
      ...tel, 
      isAudioRecording: isRecording, 
      isSpiritMode: spiritMode,
      spiritWords: spiritWords.map(w => w.word) 
    };
  }, [isRecording, spiritMode, spiritWords]);

  const toggleMic = async () => {
    if (isRecording) {
      if (recognitionRef.current) {
         try { recognitionRef.current.stop(); } catch { /* ignore stop err */ }
      }
    ...
    try {
      ...
      const AudioC = window.AudioContext || (window as unknown as { webkitAudioContext: typeof window.AudioContext }).webkitAudioContext;
      ...
      if (spiritMode) {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const windowAny = window as unknown as any;
          const SpeechRec = windowAny.SpeechRecognition || windowAny.webkitSpeechRecognition;
          if (SpeechRec) {
            const recognition = new SpeechRec();
            recognition.continuous = true;
            recognition.interimResults = true;
            recognition.onresult = (e: { results: Iterable<unknown> | ArrayLike<unknown> }) => {
              const transcript = Array.from(e.results)
                .map((res: unknown) => (res as { [key: number]: { transcript: string } })[0].transcript)
                .join(' ');
              
              const words = transcript.split(' ').filter((w: string) => w.trim().length > 0);
              if (words.length > 0) {
                const lastWord = words[words.length - 1];
                setSpiritWords(prev => {
                  const cleaned = prev.length > 5 ? prev.slice(prev.length - 5) : prev;
                  if (cleaned.length === 0 || cleaned[cleaned.length - 1].word.toLowerCase() !== lastWord.toLowerCase()) {
                    return [...cleaned, { word: lastWord, timestamp: Date.now(), id: Math.random() }];
                  }
                  return cleaned;
                });
              }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 mb-4 border-b border-[#c084fc]/20 pb-3 justify-between">
        <div className="flex items-center gap-3">
        ...
        <button 
          ...
          className={`text-[9px] font-bold tracking-widest px-2 py-1 border rounded transition-all ${
            spiritMode 
              ...
              : "bg-black/40 border-[#c084fc]/30 text-[#c084fc]/50 hover:bg-[#c084fc]/10"
          }`}
        >
          {spiritMode ? 'AI SPIRIT BOX: ON' : 'AI SPIRIT BOX: OFF'}
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative rounded-[24px] border border-[#c084fc]/20 bg-[#050505]/40 backdrop-blur-xl overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.1),0_0_30px_rgba(192,132,252,0.05)] text-white p-8">
        ...
        <div className="flex items-end justify-center w-full max-w-lg h-64 mb-12 border-b border-[#c084fc]/20 pb-4 relative z-10">
          ...
          <canvas ref={canvasRef} width={600} height={200} className="w-full h-full object-fill filter drop-shadow-[0_0_10px_rgba(192,132,252,0.6)] z-10" />
          
          {/* Spirit Words Overlay */}
          {spiritMode && isRecording && (
             <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none overflow-hidden pb-8 z-20">
               {spiritWords.map((item, idx) => (
                 <div 
                   key={item.id} 
                   className="text-3xl font-black text-white mix-blend-overlay uppercase tracking-[0.2em] animate-pulse"
                   style={{ 
                     opacity: 1 - ((spiritWords.length - 1 - idx) * 0.25),
                     transform: `scale(${1 + (idx * 0.1)}) translateY(${idx * -10}px)`
                   }}
                 >
                   {item.word}
                 </div>
               ...
               {spiritWords.length === 0 && (
                 <div className="text-[10px] text-[#c084fc]/40 tracking-widest uppercase animate-pulse">LISTENING TO THE VOID...</div>
               )}
             </div>
          )}
        </div>
      ...
      <div className="w-full h-full flex flex-col relative overflow-hidden border border-[#22d3ee]/10 rounded-2xl" style={{ background: 'radial-gradient(circle at center, #0a0a1f, #050505)' }}>
        ...
        <nav className="absolute bottom-0 left-0 right-0 bg-black/80 border-t border-[#22d3ee]/15 backdrop-blur-3xl h-[64px] flex justify-around items-center px-4 md:px-[5vw] z-30">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as 'core' | 'sensors' | 'device' | 'optics' | 'chat' | 'audio' | 'data')}
